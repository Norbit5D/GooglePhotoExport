    ...
    if file.endswith('.json'):
        ...
        if ("." + FileEnding) not in file:
            ...
            with open(INPUT_PATH + file, 'r') as datei:
                ...
                # rename files
                OutputPath = OUTPUT_PATH + "PNG_" + DtScan.strftime("%Y") + DtScan.strftime("%m") + DtScan.strftime("%d") + DtScan.strftime("%H") + DtScan.strftime("%M") + DtScan.strftime("%S") + "_" + str(FileCnt+1)
